...
def pwc_nev(t, w, t_binning):
    """
    Piecewise constant approximation of the normalized Explained Variance measure.
    ...
    return 1.0 - np.sum((w - hat_w)**2)/(np.var(w)*len(w))

def exact_nev_noise(d=10, b=3):
    """
    Computes the exact expectation of the similarity of t and the white noise
...
def exact_nev_general(C, t, A, sigma, n_bins):
    """
    Computes the exact expectation of the ssimilarity of t and the distorted t
...
def exact_nev_spherical(t, A, sigma, sigma_m, n_bins):
    """
